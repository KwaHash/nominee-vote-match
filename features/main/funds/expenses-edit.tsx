...
import { FaChevronLeft, FaFilePdf, FaImage } from 'react-icons/fa'
...
import { type Expense, type ExpenseForm, type TxType } from '@/types/funds.d'
import {
...
export default function FundsExpensesEditPage({ id }: { id: string }) {
  ...
  const receiptIsPdf = receiptUrl.toLowerCase().endsWith('.pdf')

  useEffect(() => {
    const fetchExpense = async () => {
      setIsLoading(true)
      try {
        const { data } = await axios.get<{ expense: Expense | null }>(`/api/funds/expenses/${id}`)
        ...
        if (!expense) {
          ...
          // DB stores the date as ISO yyyy-MM-dd; the form works with yyyy/MM/dd.
          setDate(expense.date.replace(/-/g, '/'))
          setCounterpart(expense.counterpart)
          setReceiptName(expense.receipt_name)
          setReceiptUrl(expense.receipt_url)
          setIsPublic(expense.is_public)
          setIsRelatedParty(expense.is_related_party)
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.put(`/api/funds/expenses/${id}`, expense)
    } catch (err) {
      const message = axios.isAxiosError<{ error?: string }>(err)
        ? err.response?.data?.error ?? '明細の更新に失敗しました。'
        : '明細の更新に失敗しました。'
      setError(message)
    }
